...
- **Frontend:** The app loads `templates/index.html`, which includes (in order) `static/js/init.js`, `static/js/core.js`, `static/js/upload.js`, `static/js/preprocess.js`, `static/js/modeling.js`, `static/js/inference.js`, and `static/js/app.js` (or a single `static/js/app.bundle.js` when `DIGITERRA_USE_JS_BUNDLE=1`). **core.js** holds shared refs, API helpers, `showTab`, and shared helpers (`formatDateTimeForFilename`, `downloadFile`, etc.). **upload.js** = upload + correlation/exploration; **preprocess.js** = preprocess form and tab; **modeling.js** = run model and results; **inference.js** = prediction and AutoML/advanced forms; **app.js** = welcome, nav, popups. New UI changes go in the appropriate feature file or core.
