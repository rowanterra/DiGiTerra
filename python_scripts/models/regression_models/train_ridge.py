...
def train_ridge(model_type, train_data, target_variables, use_stratified_split,
    ...
    model = Ridge(alpha=kwargs.get('alpha', 1.0), 
                            fit_intercept=kwargs.get('RidgeFitIntersept', True), 
                        #    normalize=kwargs.get('normalize', False), 
                            copy_X=kwargs.get('RidgeCopyX', True), 
                            max_iter=kwargs.get('RidgeMaxIter', None), 
                            tol=kwargs.get('RidgeTol', 1e-4), 
                            solver=kwargs.get('solver', 'auto'), 
                            positive=kwargs.get('RidgePositive', False), 
                            random_state=kwargs.get('RidgeRandomState', None)
                               )
